...
pub async fn help(
    ...
    ctx.send(CreateReply::default()
        .embed(serenity::CreateEmbed::new()
            ...
            .field("Komendy", "Generalnie na chwilę obecną możesz używać prawie każdej komendy oprócz hazardu jak w każdym innym bocie ekonomicznym tj. `bal`, `withdraw`, `work`, `slut`, `crime`, `deposit`, `ping`, `pay`, `rob`, `topmoney`. Są też popularne aliasy, np. `deposit` -> `dep`. To dalej alpha, więc np. większości hazardu nie ma, ale chyba będzie (raczej).", false)
            .field("Hazard", "- **automaty**: Generalnie używasz `slots` i możesz po tym podać kwotę jaką chcesz obstawić na automatach. Daje to bardzo duże zyski, ale jest mała szansa na wygraną...\n- **rzut monetą:** To jest useful w pierwszych fazach gry, ponieważ maksymalnie możesz zyskać 100 dolarów. Still warto używać jednak w początkowej fazie. Używasz tego generalnie tak, że `coinflip` i potem albo h albo t, a następnie no to ile stawiasz.", false)
