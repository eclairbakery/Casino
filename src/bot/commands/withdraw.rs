...
pub async fn withdraw(
    ctx: Context<'_>,
    #[description_localized("pl", "Kwota do wypłacenia (lub 'all')")] 
    amount_str: String,
) -> Result<(), Error> {
    ...
    if amount_to_with > member.bank {
        ctx.send(CreateReply::default()
            .embed(serenity::CreateEmbed::new()
                .title("❌ Jesteś biedny")
                .description(format!("Nie masz tyle kasy w banku, nędzarzu!\nW banku masz: `{}` 💳", member.bank))
                .color(0xFF0000))
            .ephemeral(true)
        ).await?;
        return Ok(());
    ...
    if success {
        ctx.send(CreateReply::default()
            .embed(serenity::CreateEmbed::new()
                ...
                .description(format!("Właśnie wyciągnąłeś swoje ciężko (może nie?) zarobione pieniądze."))
                .field("Kwota", format!("`{}` 💵", amount_to_with), true)
                .field("Reszta w banku", format!("`{}` 💳", member.bank - amount_to_with), true)
                .color(0xFFFF00) 
            )
        ).await?;
    } else {
        ctx.say("Bankier uciekł z Twoją kasą (błąd bazy danych).").await?;
    }
